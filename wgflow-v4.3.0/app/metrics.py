        ...
        while True:
            try:
                ...
                if tick > 0 and tick % PERSIST_EVERY == 0 and self._db is not None:
                    ...
                    # v4.2-rebuild (callback variant) used to retry a
                    # leg-2 callback here. The registration-then-bundle
                    # redesign removed callbacks entirely — there's
                    # nothing to retry. federation_links status now
                    # transitions only via the create/import-complete
                    # endpoints, never from background reconciliation.
